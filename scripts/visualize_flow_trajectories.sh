...
# Usage:
#   scripts/visualize_flow_trajectories.sh
#
# 下の変数を編集すればデフォルト値を変更できます。
# 一時的に値を変えたいだけなら、環境変数として渡すこともできます。
#   例) NUM_GEN=32 NUM_DATA=32 STEPS=100 scripts/visualize_flow_trajectories.sh
#
# 対象ラベルを絞りたい場合は LABELS にスペース区切りで指定します。
#   例) LABELS="1 3 5" scripts/visualize_flow_trajectories.sh
#
# コマンドライン引数はそのまま python スクリプトへ転送されます。
#   例) scripts/visualize_flow_trajectories.sh --dim 3 --labels 1 3 5
